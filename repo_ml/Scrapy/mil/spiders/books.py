    ...
    def parse(self, response):
        if response.url not in self.visited_urls:
            ...
            for post_link in response.xpath(
                    '//div[@class="product-card js_product"]/a/@href').extract():
                url = urljoin(response.url, post_link)
                yield response.follow(url, callback=self.parse_post)

            next_pages = response.xpath(
                    '//div[@class="pagination"]/a[contains(@class, "pagination-item") and'
                    ' not(contains(@class, "pagination-item_active"))]/@href').extract()
